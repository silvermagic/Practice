...
int main() {
    ...
    ret = amqp_basic_publish(conn,
                             ...
                             amqp_cstring_bytes("quick.orange.rabbit"), // 设置路由关键字
                             ...
                             &props,
                             amqp_cstring_bytes("Quick orange rabbit: hello world")); // 消息内容
    if (ret != AMQP_STATUS_OK) {
    ...
    ret = amqp_basic_publish(conn,
                             CHANNEL_ID, // 子频道ID
                             amqp_cstring_bytes("amq.topic"), // 主题交付模式
                             amqp_cstring_bytes("quick.orange.fox"), // 设置路由关键字
                             false, // 强制交付，表示如果RabbitMQ不能及时将消息路由给某个队列，那么将返回basic.reject方法
                             false, // 立即交付，表示如果RabbitMQ不能及时将消息路由给某个绑定在队列上的消费者，那么将返回basic.reject方法
                             &props,
                             amqp_cstring_bytes("Quick orange fox: hello world")); // 消息内容
    if (ret != AMQP_STATUS_OK) {
        std::cerr << "Sending: " << amqp_error_string2(ret) << std::endl;
        return -1;
    }
    ret = amqp_basic_publish(conn,
                             CHANNEL_ID, // 子频道ID
                             amqp_cstring_bytes("amq.topic"), // 主题交付模式
                             amqp_cstring_bytes("lazy.orange.elephant"), // 设置路由关键字
                             false, // 强制交付，表示如果RabbitMQ不能及时将消息路由给某个队列，那么将返回basic.reject方法
                             false, // 立即交付，表示如果RabbitMQ不能及时将消息路由给某个绑定在队列上的消费者，那么将返回basic.reject方法
                             &props,
                             amqp_cstring_bytes("Lazy orange elephant: hello world")); // 消息内容
    if (ret != AMQP_STATUS_OK) {
        std::cerr << "Sending: " << amqp_error_string2(ret) << std::endl;
        return -1;
    }
