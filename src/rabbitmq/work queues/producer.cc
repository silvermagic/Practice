...
int main() {
    ...
    amqp_queue_declare(conn,
                       ...
                       true, // 持久化模式，即使RabbitMQ重启消息依然不会丢失，需要交换器也为持久模式
                       false, // 独占模式，不允许创建同名独占队列，独占队列在连接关闭后自动删除（不受持久化模式影响）
                       true, // 自动删除模式，如果队列没有被任何消费者订阅就会被自动删除（设置了持久化后，当数据被处理完后将删除队列）
                       amqp_empty_table);
    reply = amqp_get_rpc_reply(conn);
    if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
    ...
    while (delay < 5)
    {
        std::string content = std::string("hello world!") + std::string(delay, '.');
        ret = amqp_basic_publish(conn,
                                 ...
                                 &props,
                                 amqp_cstring_bytes(content.c_str())); // 消息内容
        if (ret != AMQP_STATUS_OK) {
